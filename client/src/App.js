...
import { useCallOthers, useSetUsername, useSocketEventListener, useSocketInitialization } from './utils/effects';
import { sendMessage, joinRoom, leaveRoom, startPrivateCall, endPrivateCall, startGroupCall, endGroupCall } from './utils/actions';
...
function App() {
  ...
  const { socket, setSocket, peer, setPeer } = useSocketInitialization(socketUsername);
  const { runSetUsername, setRunSetUsername } = useSetUsername(socket, socketUsername);
  useSocketEventListener(socket, peer, setPeersOnConference, setAvailableUsers, setAvailableRooms, setConferenceId, setCallOthersTriggered, setTransited, setIsAnswered, selfVideoRef, remoteVideoRef, setPeerCall, setPeerCalls);
  useCallOthers(peer, callOthersTriggered, setGroupCall, setIsAnswered, peersOnConference, setPeersOnConference, setCalls);
      ...
      <div className="mt-16 container bg-[#2E4F4F]/60 backdrop-blur-[7px] max-w-[700px] h-[800px] mx-auto font-light relative overflow-hidden scroll-smooth rounded-xl">
        ...
        <Transition transited={transited} isConference>
          ...
          <div className="mt-2 flex justify-center rounded-sm hover:cursor-pointer">
            ...
            <Button
              ...
              onClick={async () =>
                groupCall
                  ...
                  : await startPrivateCall(peer, conferenceId, selfVideoRef, setIsAnswered, remoteVideoRef, setCall)
              }
              icon={<MdAddCall />}
              disabled={false}
              circle
            ...
            <Button
              className="ml-2"
              onClick={groupCall ? () => endGroupCall(calls, peerCalls, setCallOthersTriggered, setPeersOnConference) : () => endPrivateCall(call, peerCall)}
              icon={<MdCallEnd />}
              disabled={false}
              circle
              color={'#F54545'}
